...
myPages.forEach(async (page) => {
    ...
    const { default: { default: Component, getPageProps }, } = await import(path.resolve(`../client/build/pages/${pageRouteName}.bundle.js`));
    ...
    app.get(pageRoute, (req, res) => {
        const resultPage = pageHtml
            ...
            .replace('<!--myscript-->', `<script type='module' defer src='${path.join('../client/build/main/index.bundle.js')}'></script>`)
            .replace('<!--page-data-->', `<script id="PAGE_DATA" type="application/json">${JSON.stringify(props)}</script>`);
